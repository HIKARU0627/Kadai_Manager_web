...
export default function SubjectDetailPage() {
  ...
  const fetchSubjectData = async () => {
    ...
    if (result.success && result.data) {
      setSubject(result.data as SubjectDetail)
    } else {
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                ...
                <Badge variant="secondary">{subjectTypes[subject.type as keyof typeof subjectTypes]}</Badge>
              </div>
        ...
        <Tabs defaultValue="tasks" className="w-full">
          <TabsList className="grid w-full grid-cols-4">
            <TabsTrigger value="tasks">
              課題 ({subject.tasks.length})
            </TabsTrigger>
            <TabsTrigger value="events">
              テスト・予定 ({subject.events.length})
            </TabsTrigger>
            <TabsTrigger value="notes">
              授業メモ ({subject.notes.length})
            </TabsTrigger>
            <TabsTrigger value="files">
              ファイル ({subject.files.length})
            </TabsTrigger>
          </TabsList>
                          ...
                          <div className="flex items-center flex-1">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-400">
                                ...
                                {format(new Date(file.createdAt), "yyyy年MM月dd日", { locale: ja })}
                              </p>
                            </div>
                          </div>
                          <div className="flex gap-2 ml-4">
